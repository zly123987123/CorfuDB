...
public class CorfuReplicationManager {
    ...
    private final Map<CorfuReplicationSession, CorfuLogReplicationRuntime> runtimeToRemoteSession = new HashMap<>();
    ...
    private final Map<CorfuReplicationSession, LogReplicationMetadataManager> metadataManagerMap;
    ...
    public CorfuReplicationManager(LogReplicationContext context, NodeDescriptor localNodeDescriptor,
                                   Map<CorfuReplicationSession, LogReplicationMetadataManager> metadataManagerMap,
    ...
    public void start() {
        for (ClusterDescriptor remoteCluster : context.getTopology().getStandbyClusters().values()) {
            for (CorfuReplicationSubscriber subscriber : context.getConfig().getReplicationModelToStreamsMap().keySet()) {
                try {
                    startLogReplicationRuntime(remoteCluster, new CorfuReplicationSession(remoteCluster.getClusterId(),
                        subscriber));
                } catch (Exception e) {
     ...
     */
    private void connect(ClusterDescriptor remoteCluster, CorfuReplicationSession session) throws InterruptedException {
        try {
     ...
     */
    private void stopLogReplicationRuntime(CorfuReplicationSession replicationSession) {
    ...
    public void processStandbyChange(TopologyDescriptor newConfig, Set<String> sinksToAdd, Set<String> sinksToRemove,
        ...
        // Get all subscribers
        Set<CorfuReplicationSubscriber> subscribers = context.getConfig().getReplicationModelToStreamsMap().keySet();

        // Remove Sinks that are not in the new config
        for (String clusterId : sinksToRemove) {
            for (CorfuReplicationSubscriber subscriber : subscribers) {
                stopLogReplicationRuntime(new CorfuReplicationSession(clusterId, subscriber));
            }
        }
        ...
        for (String clusterId : sinksToAdd) {
            ...
            for (CorfuReplicationSubscriber subscriber : subscribers) {
                startLogReplicationRuntime(clusterInfo, new CorfuReplicationSession(clusterId, subscriber));
            }
        }
        ...
        for (String clusterId : intersection) {
            ClusterDescriptor clusterInfo = newConfig.getStandbyClusters().get(clusterId);
            for (CorfuReplicationSubscriber subscriber : subscribers) {
                runtimeToRemoteSession.get(new CorfuReplicationSession(clusterId, subscriber))
    ...
    public void enforceSnapshotSync(DiscoveryServiceEvent event) {
        CorfuLogReplicationRuntime standbyRuntime = runtimeToRemoteSession.get(
            CorfuReplicationSession.getDefaultReplicationSessionForCluster(event.getRemoteClusterInfo().getClusterId()));
